...
const sociobotKey = `sociobot_key:${productSlug}`;
const sociobotApi = 'https://api.sociobot.in/v1';
let isDemo = false;
let rows: Worklog[] = [];
...
let presetDraft = '';
let wordingDraft = '';
let wordingBefore: Array<{id:string; description:string}> = [];
let wordingMessage = '';
let drafting = false;
let wordingRun = 0;
...
const sample = () => {
  ...
  settings = {client:'Northstar Studio',invoice:'INV-2048',period:'3–14 August 2026',redact:true};
  wordingDraft = '';
  wordingBefore = [];
  wordingMessage = '';
  message='Sample worklog loaded. Change anything you need.';
};
...
const demoWording = [
  'Reviewed onboarding notes and confirmed the sprint plan.',
  'Mapped the customer handoff and listed open decisions.',
  'Built responsive page patterns for the client portal.',
  'Reviewed navigation states with the product lead.',
  'Built the account summary and invoice history screens.',
  'Corrected the mobile table layout and keyboard focus order.',
  'Prepared the release checklist and handoff notes.',
  'Documented approved revisions from the client review.',
  'Applied the approved content and spacing revisions.'
].join('\n');

function updateWordingStream(value: string, status: string) {
  wordingDraft = value;
  wordingMessage = status;
  const textarea = document.querySelector<HTMLTextAreaElement>('#wording-draft');
  const live = document.querySelector<HTMLElement>('#wording-status');
  if (textarea) textarea.value = value;
  if (live) live.textContent = status;
}

async function draftClientWording() {
  const selected = rows.filter(row => row.included);
  if (!selected.length) return;
  const token = isDemo ? '' : document.querySelector<HTMLInputElement>('#sociobot-token')?.value.trim() || '';
  if (!isDemo && !token) {
    wordingMessage = 'Paste a Sociobot key, then draft the wording.';
    render({selector:'#sociobot-token'});
    return;
  }
  if (!isDemo) localStorage.setItem(sociobotKey, token);
  const run = ++wordingRun;
  drafting = true;
  wordingDraft = '';
  wordingMessage = isDemo ? 'Loading the canned demo draft…' : 'Connecting to Sociobot…';
  render({selector:'#draft-wording'});
  try {
    if (isDemo) {
      for (const word of demoWording.split(/(?<=\s)/)) {
        if (run !== wordingRun) return;
        updateWordingStream(wordingDraft + word, 'Drafting from the canned demo response…');
        await new Promise(resolve => setTimeout(resolve, 12));
      }
    } else {
      const headers = {Authorization:`Bearer ${token}`};
      const modelsResponse = await fetch(`${sociobotApi}/models`, {headers});
      if (!modelsResponse.ok) throw new Error('Sociobot could not list models. Check the key and try again.');
      ...
      if (!model) throw new Error('No GPT-5.6 model is available for this key. Try again later.');
      const response = await fetch(`${sociobotApi}/responses`, {
        method:'POST',
        ...
        body:JSON.stringify({
          ...
          stream:true,
          input:[
            {role:'system',content:[{type:'input_text',text:'Rewrite each worklog description as a concise, factual client-facing sentence. Return one line per input, in the same order. Do not add claims or numbering.'}]},
            {role:'user',content:[{type:'input_text',text:selected.map((row,index)=>`${index + 1}. ${row.description}`).join('\n')}]}
          ]
        })
      });
      if (!response.ok || !response.body) throw new Error('Sociobot could not draft wording. Check the key and try again.');
      const reader = response.body.getReader();
      const decoder = new TextDecoder();
      let pending = '';
      while (true) {
        const {done,value} = await reader.read();
        if (run !== wordingRun) { await reader.cancel(); return; }
        ...
        for (const line of lines) {
          ...
          if (event.type === 'response.output_text.delta' && event.delta) {
            updateWordingStream(wordingDraft + event.delta, 'Sociobot is drafting. You can edit the text when it finishes.');
          }
        }
        if (done) break;
      }
    }
    if (!wordingDraft.trim()) throw new Error('Sociobot returned an empty draft. Keep your current wording or try again.');
    wordingMessage = 'Draft ready. Edit any line, then use the wording.';
  } catch (error) {
    if (run !== wordingRun) return;
    wordingDraft = '';
    wordingMessage = error instanceof Error ? error.message : 'The draft failed. Keep your current wording or try again.';
  } finally {
    if (run !== wordingRun) return;
    drafting = false;
    render(wordingDraft ? {selector:'#wording-draft'} : {selector:'#draft-wording'});
  }
}
...
function bindWork(){
  ...
  csvFile?.addEventListener('change', async event=>{
    ...
    if(!file)return;
    try { rows=parseWorklogCsv(await file.text()); wordingDraft='';wordingBefore=[];wordingMessage='';message=`Imported ${plural(rows.length, 'row')} from ${file.name}.`; save(); render(); }
  ...
  document.querySelector('#start-real')?.addEventListener('click',()=>{wordingRun++;drafting=false;isDemo=false;rows=[];settings={client:'',invoice:'',period:'',redact:true};message='Your workspace is ready. Import a CSV to begin.';route('/workspace');});
  document.querySelector('#draft-wording')?.addEventListener('click',()=>void draftClientWording());
  document.querySelector<HTMLTextAreaElement>('#wording-draft')?.addEventListener('input',event=>{wordingDraft=(event.target as HTMLTextAreaElement).value;});
  document.querySelector('#apply-wording')?.addEventListener('click',()=>{
    const selected = rows.filter(row=>row.included);
    const lines = wordingDraft.split(/\r?\n/).map(line=>line.trim()).filter(Boolean);
    if (lines.length !== selected.length) {
      wordingMessage=`Keep one non-empty line for each selected row (${selected.length} total).`;
      document.querySelector('#wording-status')!.textContent=wordingMessage;
      document.querySelector<HTMLTextAreaElement>('#wording-draft')?.focus();
      return;
    }
    wordingBefore = selected.map(row=>({id:row.id,description:row.description}));
    selected.forEach((row,index)=>{row.description=lines[index];});
    wordingDraft='';wordingMessage='Applied the draft. Review the client preview before printing.';save();render({selector:'#undo-wording'});
  });
  document.querySelector('#undo-wording')?.addEventListener('click',()=>{
    wordingBefore.forEach(previous=>{const row=rows.find(item=>item.id===previous.id);if(row)row.description=previous.description;});
    wordingBefore=[];wordingDraft='';wordingMessage='Restored the previous wording.';save();render({selector:'#draft-wording'});
  });
  document.querySelectorAll<HTMLButtonElement>('[data-edit-wording]').forEach(button=>button.addEventListener('click',()=>{
    const row=rows.find(item=>item.id===button.dataset.editWording);if(!row)return;
    const next=window.prompt('Edit the client-facing description.',row.description)?.trim();
    if(!next){message='A work description cannot be empty. Keep the current wording or enter a replacement.';render({selector:`[data-edit-wording="${row.id}"]`});return;}
    wordingBefore=rows.map(item=>({id:item.id,description:item.description}));row.description=next;wordingMessage='Updated the wording manually.';save();render({selector:'#undo-wording'});
  }));
  document.querySelector('#remove-sociobot-key')?.addEventListener('click',()=>{localStorage.removeItem(sociobotKey);wordingMessage='Removed the Sociobot key from this browser.';render({selector:'#sociobot-token'});});
  bindLicense();
...
function render(focus?: EditorFocus){
  ...
  const nextIsDemo = path === '/demo' || queryDemo;
  ...
  if(path==='/privacy') page('Privacy — Worklog Appendix','How Worklog Appendix stores data and handles optional Sociobot wording requests.','/privacy','How Worklog Appendix stores your data',`<p>Worklog Appendix reads CSV files locally. The core workflow does not send worklog rows or client details to a server.</p><h2>Local storage</h2><p>A real workspace stores its current report in this browser. Saved client details, a restored license, and an optional Sociobot key also stay here. Clear this site's browser data to delete them. Removing a license or Sociobot key in the app deletes only that key. Your report and saved client details remain. The demo stays in memory and does not read or save real workspace data.</p><h2>Optional wording drafts</h2><p>Draft client wording runs only when you choose that action. Before sending, the app shows the exact selected descriptions. It sends those descriptions and your key only to api.sociobot.in. The demo uses a canned response and makes no request. You can edit, undo, or skip every draft.</p><h2>License checks</h2><p>Buying opens Sociobot checkout. Restoring a license sends only its token to api.sociobot.in for verification, at most once every 24 hours. No CSV content or client detail is sent during a license check.</p><h2>No tracking</h2><p>No account is required for the app. There is no analytics or advertising tracker.</p>`);
  else if(path==='/terms') page('Terms — Worklog Appendix','Terms for using Worklog Appendix to prepare invoice appendices.','/terms','Terms for using Worklog Appendix',`<p>Worklog Appendix helps you prepare an invoice appendix. You are responsible for checking its wording, hours, and exported PDF before sending it.</p><h2>Optional wording drafts</h2><p>Sociobot can suggest wording when you provide a key and choose the draft action. Check every suggestion before using it. The manual workflow remains available without a key.</p><h2>One-time license</h2><p>The $19 USD license adds saved client details. Core import, redaction, invoice lines, PDF printing, and accessibility features remain free. Sociobot and Dodo are the merchant of record. A refund revokes the license automatically. A refunded or revoked license loses access to saved client details.</p><h2>No warranty</h2><p>The app is provided as-is to the extent allowed by law.</p>`);
