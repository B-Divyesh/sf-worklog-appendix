...
function bindWork(){
  ...
  csvFile?.addEventListener('keydown', event=>{
    if(event.key===' ' || event.key==='Enter'){
      event.preventDefault();
      try { csvFile.showPicker(); } catch { csvFile.click(); }
    }
  });
