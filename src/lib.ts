...
export function parseWorklogCsv(source: string): Worklog[] {
  ...
  return rows.slice(1).map((values, i) => {
    ...
    if (!/^(?:(?:0|[1-9]\d*)(?:\.\d+)?|\.\d+)$/.test(rawHours) || !Number.isFinite(hours) || hours < 0) {
      throw new Error(`Row ${i + 2} has an invalid Hours value. Use a zero or positive number (for example 0.5), then import the file again.`);
    }
...
export function plural(value: number, singular: string, pluralForm = `${singular}s`) { return `${fmt(value)} ${value === 1 ? singular : pluralForm}`; }
export function invoiceLines(rows: Worklog[]) { return groups(rows).map(g => `${g.name} — ${plural(g.hours, 'hour')}${g.amount ? ` — ${money(g.amount)}` : ''}`); }
...
export function reportMarkup(rows: Worklog[], settings: Settings) {
  const gs = groups(rows);
  const total = gs.reduce((n,g)=>n+g.hours,0);
  const esc = (value: string) => escapeHtml(safeText(value, settings.redact));
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Worklog appendix ${esc(settings.invoice)}</title><style>body{font:14px system-ui;color:#10233b;max-width:800px;margin:42px auto;padding:0 24px}h1{font:700 32px Georgia,serif}h2{margin:30px 0 8px;border-bottom:2px solid #087e8b;padding-bottom:7px}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:8px;border-bottom:1px solid #ccd8da;vertical-align:top}.num{text-align:right}small{color:#49616a}.total{font-size:18px;font-weight:700}@media print{body{margin:0}}</style></head><body><main><small>WORKLOG APPENDIX</small><h1>Completed work for ${esc(settings.client || 'Client')}</h1><p>Invoice ${esc(settings.invoice || '—')} · ${esc(settings.period || 'Billing period')}</p>${gs.map(g=>`<section><h2>${esc(g.name)} <span style="float:right">${plural(g.hours, 'hour')}</span></h2><table><thead><tr><th scope="col">Date</th><th scope="col">Completed work</th><th scope="col" class="num">Hours</th></tr></thead><tbody>${g.items.map(r=>`<tr><td>${esc(r.date)}</td><td>${esc(r.description)}</td><td class="num">${fmt(r.hours)}</td></tr>`).join('')}</tbody></table></section>`).join('')}<p class="total">Total approved work: ${plural(total, 'hour')}</p><p><small>Prepared with Worklog Appendix. Internal notes are never included in this report.</small></p></main></body></html>`;
}
